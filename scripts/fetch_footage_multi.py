...
# Old prompt versions asked GPT for a footage_query and handed it these exact
# phrases as "examples" — GPT-4o-mini regularly echoed them back verbatim
# instead of writing something specific to the story, which is why footage
# looked unrelated to the narration regardless of topic. The prompt no longer
# offers copy-pasteable example text, but this still catches any script.json
# generated by the old prompt (or a future regression) and swaps in a
# field-anchored query instead of silently fetching generic stock footage.
_GENERIC_TEMPLATE_QUERIES = {
    "dark gritty struggle failure person cinematic",
    "historical archive documentary specific scene",
    "dramatic light breakthrough moment",
    "abstract nature sky mountains sunrise",
}
